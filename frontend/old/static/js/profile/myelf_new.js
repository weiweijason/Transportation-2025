...
function updateStatistics() {
    ...
    // 計算總戰力和平均等級
    const totalPower = userCreatures.reduce((sum, creature) => 
        sum + (creature.attack || creature.power || 0), 0);
    const averageLevel = totalCount > 0 ? 
        (userCreatures.reduce((sum, creature) => sum + (creature.level || 1), 0) / totalCount).toFixed(1) : 1;
    const speciesCount = new Set(userCreatures.map(c => c.name)).size;
    
    // 更新統計面板
    updateElement('total-count', totalCount);
    updateElement('species-count', speciesCount);
    updateElement('total-power', totalPower);
    updateElement('average-level', averageLevel);
}
